...
namespace SixPlayerMod
{
    public static class PluginInfo
    {
        ...
        public const string PLUGIN_VERSION = "2.0.3"; // Verified for new game version
    }
    ...
    public class HarmonyPatches
    {
        ...
        [HarmonyPatch(typeof(LobbyController), "CheckIfAllReady")]
        [HarmonyPrefix]
        public static bool CheckIfAllReadyPrefix(LobbyController __instance)
        {
            var startGameButtonField = typeof(LobbyController).GetField("StartGameButton", BindingFlags.NonPublic | BindingFlags.Instance);
            if (startGameButtonField == null)
            {
                SixPlayerPlugin.Log.LogError("Could not find StartGameButton field on LobbyController.");
                return true;
            }
            var startGameButton = (Button)startGameButtonField.GetValue(__instance);

            var players = SteamLobby.Instance.players;
            int totalPlayers = players.Count;
            int readyPlayers = players.Count(p => p.GetComponent<ClientInstance>().Ready);

            bool canStart;
            if (totalPlayers >= 5)
            {
                canStart = (readyPlayers >= 4);
            }
            else
            {
                canStart = (readyPlayers == totalPlayers);
            }

            startGameButton.interactable = (canStart && __instance.HasEnoughPlayers() && __instance.LocalPlayerController.IsServer);
